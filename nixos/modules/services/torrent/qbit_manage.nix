...
in
{
  options.services.qbit_manage = {
    ...
    commands = mkOption {
      ...
      type = submodule {
        options = {
          ...
          tagNoHardLinks = mkEnableOption "Use this to tag torrents where the largest file does not have any hardlinks associated with any of the files outside of your Qbit_Manage root directory.";
    ...
    extraArgs = mkOption {
      type = listOf str;
      default = [ ];
      description = "Extra command line arguments to pass to qbit_manage.";
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    systemd = {
      tmpfiles.settings.qbit_manage."${cfg.dataDir}".d = {
