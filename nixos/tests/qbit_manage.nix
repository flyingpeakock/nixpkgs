...
in
{
  ...
  nodes =
    let
      ...
      environmentFile = pkgs.writeText "qbit_manage.env" ''
      ...
      qbittorrent = {
        ...
        serverConfig.Preferences.WebUI = {
          ...
          Port = lib.mkDefault (toString qbittorrent_port);
        };
      };
    in
    {
  ...
  testScript =
    ...
    ''
      ...
      def checkConfigExists(machine):
          machine.wait_for_unit("qbit_manage.service")
          machine.wait_for_file("/var/lib/qbit_manage/config.yml")

      with subtest("Check that config is created or copied when configured"):
          checkConfigExists(simple)
          checkConfigExists(declarative)

      with subtest("Check that qbit_manage connects to qBittorrent"):
